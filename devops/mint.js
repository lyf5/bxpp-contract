  ...
  try {
    ...
    const mintedTokens = await Promise.all(
      mintAssetsOnIPFS.map(async token => {
        ...
        tempPrice = '0.02';
        const tempPriceWei = utils.parseEther(tempPrice)
        const temptx =  await contract.mintCollectable(FROM, priceWei, token.name, token.path, { from: FROM, value: tempPriceWei })
        console.log("for debug. temptx: ", temptx);
        return temptx
      })
    )
